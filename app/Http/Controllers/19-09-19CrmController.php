...
$user_id = (auth()->check()) ? auth()->user()->id : null;
	...
	//	dd($Hotellccontact);
		...
		$regionalLocations = DB::table('hl_regional_locations')
				...
				->select('*','hl_corporate_contacts.hl_email as cemail','countries.name as countries','states.name as states','cities.name as cities')
			 ...
			 //  ->get();
			   // dd($country);
		  $EditHotel=Hotelcorporatecontactbasic::where('created_by',$user_id)->first();
			 
      return view('panels.crm.add_corporate',['EditHotel'=>$EditHotel,'CorporateContact'=>$CorporateContact,'Hotelagentcontactbasic'=>$Hotelagentcontactbasic,'Hotellccontact'=>$Hotellccontact,'regionalLocationsSubsidy'=>$regionalLocationsSubsidy,'regionalLocationsDirectAdd'=>$regionalLocationsDirectAdd,'regionalLocations'=>$regionalLocations,'master_lead_type'=>$master_lead_type,'users' => $users,'country' => $country,'editusers' => $users,'editcountry' => $country,'editstates' => $editstates,'editcities' => $editcities,'master_business' => $master_business,'cities' => $cities,'travel_desk' => $travel_desk,'hdcorporatecont' => $hdcorporatecont,'hdcontact_type' => $hdcontact_type]);
	...
	//dd($Hotelagentcontactbasic);
		...
		$editcities = DB::table('cities')			 
			 ->get();
      return view('panels.crm.add_agent',['regionalLocations'=>$regionalLocations,'Hotelagentcontactbasic'=>$Hotelagentcontactbasic,'master_lead_type'=>$master_lead_type,'users' => $users,'country' => $country,'editusers' => $users,'editcountry' => $country,'editstates' => $editstates,'editcities' => $editcities,'master_business' => $master_business,'cities' => $cities,'travel_desk' => $travel_desk]);
	...
	public function viewcorporate()
    {
		...
		*/	
		...
		$cor_view = array();
		$contacts_list = DB::table('hl_corporate_contacts')->get();
		//dd($contacts_list);
	...
	public function viewagent()
    {
		...
		$cor_view = array();
		$contacts_list = DB::table('hl_agents_contacts')->get();
		//dd($contacts_list);
		$contacts_list_cnt = count($contacts_list);
		if($contacts_list_cnt > 0)
		{
		...
		$editcities = DB::table('cities')			 
			 ->get();
		return view('panels.crm.edit_agent',['hotels'=>$hotels,'cor_view'=>$cor_view,'country' => $country,'editcountry' => $country,'editstates' => $editstates,'editcities' => $editcities]);
	...
	public function addHotel()
    {
		...
		$hdlead_type = DB::table('hl_master_lead_type')
			  ->get(); 
      return view('panels.crm.add_hotel',['Category'=>$Category, 'hdproperty_type'=>$hdproperty_type,'users' => $users,'country' => $country,'editusers' => $users,'editcountry' => $country,'editstates' => $editstates,'editcities' => $editcities,'hdlead_type' => $hdlead_type]);
		  ...
		  'business_name'			=> 'required',
			...
			'lead_type.required'			=> 'Lead Type is required',					
					...
					if($request->hl_ccb_id=='')
					{
						...
						$Hotellccontact->hl_landline=$request->landline;
						...
						$hlccid = $Hotellccontact->id;
							...
							$Hotellccontact = Hotelcorporatecontactbasic::where('hl_ccb_id', $request->hl_ccb_id)
								->update([							
									...
									'hl_travel_desk_type'=>$request->h_desk_type,
								]);	
		...
		/*
		  $this->validate($request,
			...
			*/						 
				...
				$Hotellccontact->created_by=auth()->user()->id;				
				$Hotellccontact->hl_business_name=$request->business_name;
				$Hotellccontact->hl_agent_type=$request->business_type;					
		  ...
		  $this->validate($request,
			...
			*/						 
				...
				for($gg = 0; $gg < $cnt; $gg++)
				{
					...
					$Hotellccontact_r->hl_subsid_of=$request->subs_of1[$gg];					    
