...
use App\Models\Hlagentmulticontact;
   ...
   // foreach($cnt_regional as $cnt_regional1){
      foreach($checkedVals as $chk_val)
      {
        ...
        $sleadsA = DB::table('hotel_leads')
          ...
          ->get();
          //array_push($array_dataA,$sleadsA);
        }
      else if($chk_val=='2'){
        ...
        //->where('hl_master_regional.hl_ms_r_id', $cnt_regional1->hl_ms_r_id)
        ->get();
         // array_push($array_dataB,$sleadsB);
...
//outlook controller
   public function events()
  {
    $acc_token="";
    $user_id = (auth()->check()) ? auth()->user()->id : null;
    $UserMailSetting=UserMailSetting::where('user_id',$user_id)->first();
    if($UserMailSetting){
    $email = $UserMailSetting->email;
    $password=base64_decode($UserMailSetting->password);


    $curl = curl_init();
    curl_setopt_array($curl, array(
    CURLOPT_URL => "https://login.microsoftonline.com/ap-lbc.com/oauth2/token",
    ...
    CURLOPT_POSTFIELDS => "client_id=d9f2f75b-1649-43b3-bf85-e119f1463102%0A&client_secret=IK-exv3g303xdGuyKpiPeDnjAmkhl%3A%3D%3A&username=".$email."&password=".$password."&grant_type=password&resource=https%3A%2F%2Fgraph.microsoft.com&scope=openid",
    CURLOPT_HTTPHEADER => array(
    "cache-control: no-cache",
    "content-type: application/x-www-form-urlencoded",
    "postman-token: c2392ed1-fa6c-eb84-0fe9-d3eefa29819e"
    ),
    ));
    ...
    if ($err) {
     return redirect('crm/settings')->with('message','Please enter a valid outlook email id and password');
    //dd("cURL Error #:" . $err);
    } else {
    ...
    return redirect('crm/settings')->with('message','Please enter a valid outlook email id and password');
          ...
          $_SESSION['acc_token'] =$data['access_token'];
          }else{
          $_SESSION['acc_token'] ='';
          //dd($_SESSION['acc_token']);   
          }
...
//insertion in outlook from crm
    ...
    if($out_event->attendee_mail){
      $items='{"EmailAddress": {"Address": "'.$out_event->attendee_mail.'","Name": ""},"Type": "Required"}';
    }else{
    $data=explode(',', $out_event->hle_contacts);
    $items = array();
    foreach ($data as $key => $value) { 
    $user=User::where('id',$value)->first();
    $items[]='{"EmailAddress": {"Address": "'.$user->email.'","Name": "'.$user->first_name.' '.$user->last_name.'"},"Type": "Required"}';
    ...
    //dd($token);
      ...
      $data2='{"Subject": "'.$out_event->hle_title.'","Body": {"ContentType": "HTML","Content": "'.$out_event->hle_description.'"},"Start": {"DateTime": "'.$start.'","TimeZone": "'.$out_event->hle_timezone.'"},"End": {"DateTime": "'.$end.'","TimeZone": "'.$out_event->hle_timezone.'"},"location":{"displayName":"'.$out_event->hle_location.'"},"Attendees": '.$items.'}';
...
//for delete
 ...
 // $tokenCache = new \App\TokenStore\TokenCache;
  $graph = new Graph();
 $graph->setAccessToken($acc_token);
  $user = $graph->createRequest('GET', '/me')
                ->setReturnType(Model\User::class)
                ->execute();
  $eventsQueryParams = array (
...
$user_id = (auth()->check()) ? auth()->user()->id : null;
  ...
  $cnt_regional =  DB::table('hl_master_regional')->get();
    ...
    return view('panels.crm.events', ['out_events'=>$out_events,'tasks'=>$tasks,'tasks1'=>$tasks1,'hl_business_name'=>$hl_business_name,'hl_corporate_name'=>$hl_corporate_name, 'task_types'=>$task_types, 'task_types1'=>$task_types1, 'task_for'=>$task_for, 'users'=>$users, 'today_tasks'=>$today_tasks, 'activity'=>$activity, 'applicable_to'=>$applicable_to, 'applicable_to1'=>$applicable_to1, 'sleads'=>$sleads,'nextstep'=>$nextstep, 'outcomes'=>$outcomes]);
     ...
     public function getAgBusiname(Request $request)
  {
    $business_name = DB::table('hl_agents_contacts_basic')
      ->where('hl_business_name', '=', $request->b_name)
      ->where('hl_agentcont_id','!=', $request->b_id)
      ->count();
    
    return response()->json($business_name);
  }

  public function getCorpBusiname(Request $request)
  {
    $business_name = DB::table('hl_corporate_contact_basic')
      ->where('hl_business_name', '=', $request->b_name)
      ->where('hl_ccb_id','!=', $request->b_id)
      ->count();
...
public function getcontacts(Request $request)
 ...
 //dd(DB::getQueryLog());
  ...
  public function settings()
  {
    ...
    $user_id = (auth()->check()) ? auth()->user()->id : null;
    $USetting=UserMailSetting::where('user_id',$user_id)->first();

     
       $Contact_Designation=Contactdesignation::get();

    return view('panels.crm.settings', compact('USetting','Contact_Designation'),['users'=>$users,'master_nextstep'=>$master_nextstep, 'master_outcomes'=>$master_outcomes,'master_tasks'=>$master_tasks, 'master_regional'=>$master_regional, 'sa_regional'=>$sa_regional, 'na_regional'=>$na_regional, 'eur_regional'=>$eur_regional , 'mea_regional'=>$mea_regional, 'apac_regional'=>$apac_regional, 'region'=>$region, 'master_leadtypes'=>$master_leadtypes, 'region1'=>$region1, 'task_for'=>$task_for, 'app_to'=>$app_to, 'activity_for'=>$activity_for,'user_settings'=>$user_settings,'hotel_group'=>$hotel_group,'templates'=>$templates]);
  }
  ...
  public function store(Request $request)
  {
    ...
    $Events->hle_status=1;
    $Events->outlook_status=$outlook_status;
    $Events->hle_assigned_to = $user_id;
    $Events->outlook_id = $outlook_id;
  ...
  public function ajaxuseremailsettingadd(Request $request){

    $user_id = (auth()->check()) ? auth()->user()->id : null;

$password=base64_encode($request->outlook_password);

    if($request->id == 0){
        
        $Users = new UserMailSetting;
        $Users->user_id=$user_id;
        $Users->email=$request->outlook_email;
        $Users->password=$password;
        $Users->mail_type=1;
        $Users->save();
        $UserSetting_id =$Users->id;   
    
        
    }else{
     
    $UserSetting=UserMailSetting::where('id', $request->id)
    ->update(['user_id' =>$user_id,'email' =>$request->outlook_email,'password' =>$password,'mail_type' =>'1']); 
    $UserSetting_id = 'updated'; 
...
$user_id = (auth()->check()) ? auth()->user()->id : null;
  ...
  public function remove_agContact(Request $request)
    {
      //DB::enableQueryLog();
      
        DB::table('hl_agents_multi_contacts')
        ->where('hl_multi_id', $request->id)
        ->update([              
        'hl_status' =>2
        ]); 
      $remove='Contact Removed Successfully';
  ...
  public function getagent_loc(Request $request)
    {
      //DB::enableQueryLog();
       ...
       $hle_agent_loc = DB::table('hl_agents_contacts_basic')
       ->select('*', 'states.name as states','cities.name as cities', 'countries.name as country')
       ->leftJoin('countries', 'countries.id', '=', 'hl_agents_contacts_basic.hl_country')
    ...
    public function getuser(Request $request)
    {
      ...
      ->first(); 
      return '{"view_details": ' . json_encode($user) . '}';
    } 
  public function getcorporate_loc(Request $request)
    {
      //DB::enableQueryLog();
       ...
       $hle_cor_loc = DB::table('hl_corporate_contact_basic')
       ->select('*', 'states.name as states','cities.name as cities', 'countries.name as country')
       ->leftJoin('countries', 'countries.id', '=', 'hl_corporate_contact_basic.hl_country')
...
//dd(DB::getQueryLog());
  ...
  public function getsubsidyAddress(Request $request)
    {
       ...
       // ->get();   
        ...
        $Hlcorporatecontact = DB::table('hl_agent_regional_locations')
  ...
  //  dd($Hotellccontact);
    ...
    $regionalLocations = DB::table('hl_regional_locations')
        ...
        ->leftJoin('hl_agent_regional_locations', 'hl_agent_regional_locations.hl_regl_id', '=', 'hl_outplant.hl_agent_reg_id')
...
//          }
      ...
      $associate_c=DB::table('hl_corporate_contact_basic')
       ->select('*','hl_corporate_contact_basic.hl_business_name as business_name')
       ->leftJoin('hl_master_business', 'hl_master_business.hl_mt_bus_id', '=', 'hl_corporate_contact_basic.hl_business_type')
       ->where('hl_master_business.hl_business_name','=','Associations')
       ->get();
        $associate_a=DB::table('hl_agents_contacts_basic')
        ->select('*')
        ->leftJoin('hl_business_agent_type', 'hl_business_agent_type.h_main_id', '=', 'hl_agents_contacts_basic.hl_agentcont_id')
       ->where('hl_business_agent_type.h_agent_type_id','=',7)
       ->get();
       
      return view('panels.crm.add_corporate',compact('master_lead_type','hl_corporate_contact_basic','hl_agents_contacts_basic','country','editstates','editcities','Contactdesignation','Outplant','Hotellccontact'),['CorporateContact'=>$CorporateContact,'Hotelagentcontactbasic'=>$Hotelagentcontactbasic,'Implant'=>$Implant,'regionalLocationsDirectAdd'=>$regionalLocationsDirectAdd,'regionalLocations'=>$regionalLocations,'users' => $users,'editusers' => $users,'editstates' => $editstates,'editcities' => $editcities,'master_business' => $master_business,'cities' => $cities,'travel_desk' => $travel_desk,'associate_c' => $associate_c,'hdcorporatecont' => $hdcorporatecont,'hdcontact_type' => $hdcontact_type,'associate_a' => $associate_a]);
...
//dd($CorporateContact);
    ...
    $cities =  DB::table('cities')->get();
      ...
      $Hotelagentcontactbasic1=Hotelagentcontactbasic::where('created_by',$user_id)->get();
      $Contactdesignation=Contactdesignation::get();
...
//DB::enableQueryLog();
    $hlagentscontacts = DB::table('hl_agent_regional_locations')
        ->select('*','hl_agents_contacts.hl_email as cemail','countries.name as countries','states.name as states','cities.name as cities','hl_agents_contacts.hl_corcont_id as contacts_id')
        ->leftJoin('hl_agents_contacts', 'hl_agents_contacts.hl_agentcont_id','=','hl_agent_regional_locations.hl_regl_id')
        ->leftJoin('countries', 'countries.id', '=', 'hl_agent_regional_locations.hl_country')
        ->leftJoin('states', 'states.id', '=', 'hl_agent_regional_locations.hl_state')
        ->leftJoin('cities', 'cities.id', '=', 'hl_agent_regional_locations.hl_city')
...
//dd(DB::getQueryLog());
    ...
    $Outplant = DB::table('hl_regional_locations')
        ...
        $agenttype=DB::table('hl_business_agent_type')
                  ->select('*')
                  ->leftJoin('hl_agents_contacts_basic', 'hl_agents_contacts_basic.hl_agentcont_id', '=', 'hl_business_agent_type.h_main_id')
                  ->where('hl_agents_contacts_basic.hl_agentcont_id',$id)
                  ->get();
       ...
       ->where('hl_business_agent_type.h_agent_type_id','=',7)
       ->get();
       $contactD = Hlagentmulticontact::where(['hl_agent_id'=>$id,'hl_status'=>1])->get()->toArray();

     return view('panels.crm.edit_agent_business',compact('Implant','Hotelagentcontactbasic1','Outplant','hlagentscontacts','Contactdesignation','hl_corporate_contact_basic','hl_agents_contacts_basic'),['Hotelagentcontactbasic'=>$Hotelagentcontactbasic,'master_lead_type'=>$master_lead_type,'users' => $users,'country' => $country,'editusers' => $users,'editcountry' => $country,'editstates' => $editstates,'editcities' => $editcities,'master_business' => $master_business,'cities' => $cities,'travel_desk' => $travel_desk,'agenttype' => $agenttype,'associate_c' => $associate_c,'associate_a' => $associate_a,'contactD' => $contactD]);   
     

    }


     public function importAgentLocation(Request $request){



        if($request->hasFile('agent_off_location_file')){

            $path = $request->file('agent_off_location_file')->getRealPath();

            $data = Excel::load($path)->get();


            $created_by=auth()->user()->id;
            $hl_id=$request->import_hl_agentcont_id;
            ...
            if($data->count()){
                ...
                foreach ($data as $key => $value) {
                 ...
                 // $Country=Country::where('LOWER(name)', 'like', '"%' . strtolower($value->hl_country) . '%"')->first();
                DB::enableQueryLog();
                $Country=Country::where(DB::raw('TRIM(LOWER(name))'), 'LIKE', '%' . trim(strtolower($value->hl_country)) . '%')->first();
                
                  
                  if($Country){

                    $hl_country=$Country->id;
                        $States=States::where(DB::raw('TRIM(LOWER(name))'), 'LIKE', '%' . trim(strtolower($value->hl_state)) . '%')->first();
                      if($States){
                          $hl_state=$States->id;              
                          $Cities=Cities::where(DB::raw('TRIM(LOWER(name))'), 'LIKE', '%' . trim(strtolower($value->hl_city)) . '%')->first();
                          if($Cities){
                  ...
                  if($value->hl_lead_type =="Website"){
                    $hl_lead_type=1;
                  }elseif($value->hl_lead_type =="Sales team"){
                    $hl_lead_type=2;
                  }elseif($value->hl_lead_type =="Event"){
                    $hl_lead_type=3;
                  }elseif($value->hl_lead_type =="LinkedIn"){
                    $hl_lead_type=4;
                  }elseif($value->hl_lead_type =="Email"){
                    $hl_lead_type=5;
                  }else{
                    $hl_lead_type='';
                  }

                  if($value->hl_ofz_address!='')
                  {
                    $arr = ['created_by' => $created_by,
                    'iata_number'=> $value->iata_number,
                    'hl_ofz_address'=> $value->hl_ofz_address,
                    'hl_country'=> $hl_country,
                    'hl_state'=> $hl_state,
                    'hl_city'=> $hl_city,
                    'hl_postcode'=> $value->hl_postcode,
                    'hl_ofz_no'=> $value->hl_ofz_no,
                    'hl_email'=> $value->hl_email,
                    'hl_lead_type'=> $hl_lead_type,
                    'hl_loc_type'=> $value->hl_loc_type,                    
                    'hl_agent_basic_id'=> $hl_id];

                    $AgentLocationId=DB::table('hl_agent_regional_locations')->insertGetId($arr);
                 
                  }

                  $hl_cont_design='';

                    if($value->hl_cont_design!='')
                    {
                   // DB::enableQueryLog();
                    $Contactdesignation=Contactdesignation::where(DB::raw('TRIM(LOWER(hl_title))'), 'LIKE', '%' . trim(strtolower($value->hl_cont_design)) . '%')->first();
                      if($Contactdesignation){
                        $hl_cont_design=$Contactdesignation->hl_cd_id;
                      }
                    }

                    $cont = ['created_by' => $created_by,
                      'hl_title'=> $value->hl_title,
                      'hl_first_name'=> $value->hl_first_name,
                      'hl_last_name'=> $value->hl_last_name,
                      'hl_cont_design'=> $hl_cont_design,
                      'hl_mob_no'=> $value->hl_mob_no,
                      'hl_email'=> $value->con_hl_email,
                      'hl_linked_in'=> $value->hl_linked_in,
                      'hl_skype'=> $value->hl_skype,
                      'hl_dob'=> date("Y-m-d", strtotime($value->hl_dob)),                    
                      'hl_event_invite'=> $value->hl_event_invite,
                      'hl_agentcont_id'=> $AgentLocationId];

                      $AgentConId=DB::table('hl_agents_contacts')->insertGetId($cont);

                }

                               
                      
                return back()->with('sms','Agent Office Location Data Inserted successfully');
              

            }

        }
 return back()->with('sms','Request data does not have any files to import.');
      //  dd('Request data does not have any files to import.');      
 ...
 public function getagentlist()
    ...
    public function getAgents(Request $request)
    {
      ...
      $data['hlagentscontacts'] = DB::table('hl_agents_contacts')
        ...
        ->first();
      $data['subsidy']=DB::table('hl_subsidy')
        ->select('*')
        ->leftJoin('hl_agents_contacts', 'hl_agents_contacts.hl_agentcont_id','=','hl_subsidy.h_main_id')   
        ->where('hl_agents_contacts.hl_corcont_id',$id)   
        ->get();        
      $data['states'] = DB::table('states')
      ->where('country_id',$data['hlagentscontacts']->hl_country)      
  ...
  public function deleteAgents(Request $request)
    {
     ...
     $Del=DB::table('hl_agent_regional_locations')->where('hl_regl_id',$id)->delete();
      
      $data="success";
     return '{"view_details": ' . json_encode($data) . '}';
  }

   public function deleteSubAgents(Request $request)
    {
       $id=$request->id;        

     $Del=DB::table('hl_agents_contacts')->where('hl_corcont_id',$id)->delete();
    ...
    public function corporateUpdated(Request $request)
  {
      ...
      'lead_type.required'      => 'Lead Type is required',         
        ...
        $user_id = (auth()->check()) ? auth()->user()->id : null;
        if($request->associate_with!=''){
        $associate_with=implode(',', $request->associate_with);
        }else{
          $associate_with="";
        }
  ...
  public function agentUpdated(Request $request)
  {
      ...
      'lead_type.required'      => 'Lead Type is required',         
        ...
        $user_id = (auth()->check()) ? auth()->user()->id : null;
        if($request->subsidy_of_ag!=''){
        $subsidy_of_ag=implode(',', $request->subsidy_of_ag);
        }else{
        $subsidy_of_ag="";
        }   
        ...
        $cnt1 = count($request->m_con_first_name);
        for($gg = 0; $gg < $cnt1; $gg++)
        {    

            if($request->hl_multi_id[$gg]!=''){
              $dat="ag_associate_with".$gg;
              if($request->$dat!=''){
              $ag_associate_with=implode(',', $request->$dat);
              }else{
              $ag_associate_with="";
              }

              DB::table('hl_agents_multi_contacts')
              ->where('hl_multi_id', $request->hl_multi_id[$gg])
              ->update([              
              'm_con_contact_designation' =>$request->m_con_contact_designation[$gg],
              'm_con_title' =>$request->m_con_title[$gg],
              'm_con_first_name' =>$request->m_con_first_name[$gg],
              'm_con_last_name' =>$request->m_con_last_name[$gg],
              'm_con_mobile_no' =>$request->m_con_mobile_no[$gg],
              'm_con_landline_no' =>$request->m_con_landline_no[$gg],
              'm_con_email' =>$request->m_con_email[$gg],
              'm_con_linkedin_contact' =>$request->m_con_linkedin_contact[$gg],
              'm_con_skype_contact' =>$request->m_con_skype_contact[$gg],
              'm_con_dob' =>$request->m_con_dob[$gg],
              'ag_associate_with' =>$ag_associate_with,
              ]);
              }else{
              $dat="ag_associate_with".$gg;
              if($request->$dat!=''){
              ...
              if($request->m_con_first_name[$gg]!=''){
              $Hotellccontact_c = new Hlagentmulticontact;       
              $Hotellccontact_c->created_by=$user_id;
              $Hotellccontact_c->hl_agent_id=$hlccid;    
              $Hotellccontact_c->m_con_contact_designation=$request->m_con_contact_designation[$gg];
              $Hotellccontact_c->m_con_title=$request->m_con_title[$gg];         
              $Hotellccontact_c->m_con_first_name=$request->m_con_first_name[$gg];
              $Hotellccontact_c->m_con_last_name=$request->m_con_last_name[$gg];
              $Hotellccontact_c->m_con_mobile_no=$request->m_con_mobile_no[$gg];
              $Hotellccontact_c->m_con_landline_no=$request->m_con_landline_no[$gg];
              $Hotellccontact_c->m_con_email=$request->m_con_email[$gg];
              $Hotellccontact_c->m_con_linkedin_contact=$request->m_con_linkedin_contact[$gg];
              $Hotellccontact_c->m_con_skype_contact=$request->m_con_skype_contact[$gg];
              $Hotellccontact_c->m_con_dob=$request->m_con_dob[$gg];
              $Hotellccontact_c->ag_associate_with=$ag_associate_with;
              $Hotellccontact_c->hl_status=1;
              $Hotellccontact_c->save();        
              $hlccidP = $Hotellccontact_c->id; 
              }
              }
          ...
          Hotelbusinessagenttype::where('h_main_id',$hlccid)->delete();
          foreach (explode(',', $sub) as $value) {
  ...
  public function viewcorporate($id=null)
    {       
      ...
      $hotelsCont = Cache::remember('hotelcorporatecontactbasic',5,function(){
        return Hotelcorporatecontactbasic::orderBy('hl_business_name', 'asc')->get();
        }); 

      return view('panels.crm.edit_corporate',compact('hotelsCont'));
    }
  ...
  public function viewagent()
    {           
     ...
     // ->get(); 
     $hotelsCont = Cache::remember('hotelscontactbasic',5,function(){
        return Hotelagentcontactbasic::orderBy('hl_business_name', 'asc')->get();
        });  
    return view('panels.crm.edit_agent',compact('hotelsCont')); 
    }
  ...
  public function corporateadded(Request $request)
  {
    //'landline.required'     => 'Landline number is required',
      ...
      'lead_type.required'      => 'Lead Type is required',         
        ...
        if($request->associate_with!=''){
        $Hotellccontact->associate_with=implode(',', $request->associate_with);}
        $Hotellccontact->save();
  ...
  public function corregional_added(Request $request)
  {
    ...
    /*
      ...
      'lead_type.required'      => 'Lead Type is required',         
        ...
        if($request->subsidy_new_ofz!=''){
        $subsidy_new_ofz=implode(',', $request->subsidy_new_ofz);
        }else{
        $subsidy_new_ofz="";
        }
        ...
        $Hotellccontact_r->subsidy_of=$subsidy_new_ofz;
        ...
        for($gg = 0; $gg < $cnt; $gg++)
        {       
          $dat="associate_withM".$gg;
          if($request->$dat!=''){
          $associate_withM=implode(',', $request->$dat);
          }else{
          $associate_withM="";
          }
          ...
          $Hotellccontact_c->associate_withM=$associate_withM;
          $Hotellccontact_c->save();        
        ...
        if($request->subsidy_of!=''){
        $subsidy_of=implode(',', $request->subsidy_of);
        }else{
        $subsidy_of="";
        }
        if($request->associate_withC!=''){
        $associate_withC=implode(',', $request->associate_withC);
        }else{
        $associate_withC="";
        }
        if($request->ofz_type == 'Implant'){
          ...
          $Hlconsortiaofficeaddress->subsidy_of=$subsidy_of;
          ...
          $Hlconsortiacontacts->associate_with=$associate_withC;          
...
//DB::enableQueryLog();
    ...
    //dd(DB::getQueryLog());  
        ...
        if($request->ofz_type == 'Implant'){
          ...
          for($i=0;$i<$da; $i++)
          {
          $val=explode('_', $request->subsidy_ofz2[$i]);
          $Hloutplant = new Hloutplant;       
          $Hloutplant->hl_regl_m_id=$id;
          $Hloutplant->hl_agent_cor_id=$val[0];        
          $Hloutplant->hl_agent_reg_id=$val[1];        
          $Hloutplant->save();      
  ...
  public function agentadded(Request $request)
  {
      ...
      'website.required'      => 'Website is required'    
        ...
        $Hotellccontact->hl_website=$request->website;
        $Hotellccontact->hl_lead_type=$request->lead_type;      
        $Hotellccontact->subsidy_of=$subsidy_of_ag;
        //$Hotellccontact->hl_subsidiary_of=$request->subs_of;
  ...
  public function ageregional_added(Request $request)
  {
      ...
      'lead_type1.required'     => 'Lead type is required'    
           ...
           if($hl_agentcont_id==''){
            $hl_agentcont_id=$request->busi_id;
           }
            DB::enableQueryLog();      
          $userid = auth()->user()->id;     
          ...
          $associate_withM="";
            ...
            $Hotellccontact_c->hl_skype=$request->skype_contact[$gg];
            $Hotellccontact_c->hl_dob=date('Y-m-d',strtotime($request->dob[$gg]));
            $Hotellccontact_c->associate_with=$associate_withM;
            $Hotellccontact_c->hl_event_invite=$request->invite[$gg];
...
$hlccid=$request->agtcont_mid;
          ...
          if(!empty($request->edit_subsidy_m_off))
          {
          $sub=implode(',', $request->edit_subsidy_m_off);
          Hotelsubsidy::where('h_main_id',$hlccid)->delete();
          foreach (explode(',', $sub) as $value) {
          $val=explode('_', $value);
          ...
          $cnt = count($request->edit_first_name);
          for($gg = 0; $gg < $cnt; $gg++)
          {
          $datq="associate_withEdit".$gg;
          if($request->$datq!=''){
          $associate_withE=implode(',', $request->$datq);
          }else{
          $associate_withE="";
          }
          if($request->agtcont_cid[$gg]!='')
          {
            ...
            ->update([
            'hl_title'=>$request->edit_title[$gg],
            'hl_first_name'=>$request->edit_first_name[$gg], 
            'hl_last_name'=>$request->edit_last_name[$gg],
            'hl_cont_design'=>$request->edit_contact_designation[$gg],
            'hl_type_of_cont'=>$request->contact_type[$gg],
            'hl_mob_no'=>$request->edit_mobile_no[$gg],
            'hl_email'=>$request->edit_email[$gg],
            'hl_linked_in'=>$request->edit_linkedin_contact[$gg],
            'hl_skype'=>$request->edit_skype_contact[$gg],
            'associate_with'=>$associate_withE,
            'hl_dob'=>date('Y-m-d',strtotime($request->edit_dob[$gg])),
            'hl_event_invite'=>$request->edit_event_invite[$gg]
            ]); 

          }else{  
            $Hotellccontact_c = new Hlagentcontact;       
            $Hotellccontact_c->created_by=auth()->user()->id;     
            $Hotellccontact_c->hl_title=$request->edit_title[$gg];
            $Hotellccontact_c->hl_first_name=$request->edit_first_name[$gg];     
            $Hotellccontact_c->hl_last_name=$request->edit_last_name[$gg];
            $Hotellccontact_c->hl_cont_design=$request->edit_contact_designation[$gg];
            $Hotellccontact_c->hl_type_of_cont=$request->contact_type[$gg];
            $Hotellccontact_c->hl_mob_no=$request->edit_mobile_no[$gg];
            $Hotellccontact_c->hl_email=$request->edit_email[$gg];
            $Hotellccontact_c->hl_linked_in=$request->edit_linkedin_contact[$gg];
            $Hotellccontact_c->hl_skype=$request->edit_skype_contact[$gg];
            $Hotellccontact_c->hl_dob=date('Y-m-d',strtotime($request->edit_dob[$gg]));
            $Hotellccontact_c->hl_event_invite=$request->edit_event_invite[$gg];
            $Hotellccontact_c->associate_with=$associate_withE;
            $Hotellccontact_c->hl_agentcont_id=$hlccid;
            $Hotellccontact_c->save();
          }       
...
$hlccid=$request->implcont_mid;
        ...
        if($request->implsubsidy_of_edit!=''){
        $implsubsidy_of_edit=implode(',', $request->implsubsidy_of_edit);
        }else{
        $implsubsidy_of_edit="";
        }
        if($request->implassociate_withEdit!=''){
        $implassociate_withEdit=implode(',', $request->implassociate_withEdit);
        }else{
        $implassociate_withEdit="";
        }
          ...
          'subsidy_of'=>"$implsubsidy_of_edit",       
          ]);
          ...
          if($request->implcont_cid !='')
          {
            ...
            'associate_with'=>$implassociate_withEdit,
...
$hlccid=$request->corpcont_mid;
          ...
          //dd($request->ecorp_location_type1);
          if($request->corpsubsidy_of!=''){
          $corpsubsidy_of=implode(',', $request->corpsubsidy_of);
          }else{
          $corpsubsidy_of="";
          }
          ...
          if($request->corpcont_cid[$gg] !='')
          {
            ...
            'associate_withM'=>$associate_withE,
            ...
            $Hotellccontact_c->hl_reg_loc_id=$hlccid;
            $Hotellccontact_c->associate_withM=$associate_withE;
            $Hotellccontact_c->save();
          }       
