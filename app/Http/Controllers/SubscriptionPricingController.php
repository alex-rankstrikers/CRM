...
class SubscriptionPricingController extends Controller
{
    ...
    {		
       ...
       return view('panels.admin.subscription.pricing',['pricing'=>$pricing,'currency'=>$currency,'country'=>$country,'plan'=>$plan,'duration'=>$duration,'features'=>$features,'edit_features'=>$features,'edit_country'=>$country,'edit_plan'=>$plan,'edit_duration'=>$duration]);
    }
	...
	{	 
		...
		$this->validate($request,
            ...
            [
				...
				->where('id', $request->id)
				->update(['c_id' => $request->country,'c_id' => $request->currency,'f_id' => implode(',',$request->features),'p_id' => $request->plan,'d_id' => $request->d_id,'price' => $request->price,]);
