...
use App\Models\CategoryRoomsFeatures;
use App\Models\CategoryRoomsView;
use App\Models\HotelRoomsKeyFeatures;
use App\Models\HotelRoomsView;
...
$user_id = (auth()->check()) ? auth()->user()->id : null;
    ...
    {
    $user_id = (auth()->check()) ? auth()->user()->id : null;
    $hotels = Hotels::select('*','hotels.id as hid')
    ->leftJoin('hotel_group','hotel_group.id','=','hotels.grp_name')
    ->leftJoin('hodel_address','hodel_address.hl_id','=','hotels.id')
    ->orderBy('hotel_group.title', 'desc')    
    ->get(); 
...
public function portalAdd(Request $request)
    {
        $user_id = (auth()->check()) ? auth()->user()->id : null;
        //$venue= Venue::all();
        $country= Country::all();
...
$RoomBedType=RoomBedType::all();
        ...
        $HotelGroup= HotelGroup::where('status','1')->get();
      ...
      return view('panels.hotelier.hotel_configuration_add',compact('HotelGroup','states','cities','RoomClass','RoomBedType','RoomsCategory','Currencylist','country','Zone','Airports','CategoryTaxType','CategoryMealPlan','CategoryPropertyType','CategoryLocationType','Benefits','Amenities','Business','VenueFeatures','VenueFoodDrink','VenueLicensing','VenueCapacity'),['Category'=>$Category,'users' => $users,'editusers' => $users,'editcountry' => $country,'editstates' => $states,'editcities' => $cities]);
    }

     public function portadded(Request $request)
    {

    $vid=$request->hotel_mid;
    $hl_id=$request->hotel_mid;
    $crm_path=env('HOTEL_PATH');
    $crmdestinationPath = $crm_path.'/uploads/';
    $web_path=env('WEB_PATH');
    $webdestinationPath = $web_path.'/uploads/';
    ...
    if($uploadFile){ 
        ...
        copy($destinationPath.'/'.$fd_uploadFile, $crmdestinationPath.'/hotels/'.$fd_uploadFile);
        //copy($destinationPath.'/'.$fd_uploadFile, $webdestinationPath.'/hotels/'.$fd_uploadFile);
    ...
    if($uploadFile1){                
        ...
        copy($destinationPath.'/'.$fd_uploadFile1, $crmdestinationPath.'/hotels/'.$fd_uploadFile1);
        //copy($destinationPath.'/'.$fd_uploadFile1, $webdestinationPath.'/hotels/'.$fd_uploadFile1);
    }else{
    $fd_uploadFile1 ="";
    }
    ...
    if($uploadFile2){                
        ...
        copy($destinationPath.'/'.$fd_uploadFile2, $crmdestinationPath.'/hotels/'.$fd_uploadFile2);
        //copy($destinationPath.'/'.$fd_uploadFile2, $webdestinationPath.'/hotels/'.$fd_uploadFile2);
    }else{
    $fd_uploadFile2 ="";
    }
    ...
    if($uploadFile3){                
        ...
        copy($destinationPath.'/'.$fd_uploadFile3, $crmdestinationPath.'/hotels/'.$fd_uploadFile3);
        //copy($destinationPath.'/'.$fd_uploadFile3, $webdestinationPath.'/hotels/'.$fd_uploadFile3);
    }else{
    $fd_uploadFile3 ="";
    }
    ...
    if($uploadFile4){                
        ...
        copy($destinationPath.'/'.$fd_uploadFile4, $crmdestinationPath.'/hotels/'.$fd_uploadFile4);
        //copy($destinationPath.'/'.$fd_uploadFile4, $webdestinationPath.'/hotels/'.$fd_uploadFile4);
    }else{
    $fd_uploadFile4 ="";
    }
    ...
    if($uploadFile5){                
        ...
        copy($destinationPath.'/'.$fd_uploadFile5, $crmdestinationPath.'/hotels/'.$fd_uploadFile5);
        //copy($destinationPath.'/'.$fd_uploadFile5, $webdestinationPath.'/hotels/'.$fd_uploadFile5);
    }else{
    $fd_uploadFile5 ="";
    }

    $Hotels = new Hotels;
    $Hotels->user_id=auth()->user()->id;
    $Hotels->category_id='';
    $Hotels->grp_name=$request->grp_name;
    $Hotels->hotel_name=$request->hotel_name;
    $Hotels->hotel_id=$request->hotel_id;
    $Hotels->hotel_code=$request->hotel_code;
    $Hotels->pms_code=$request->pms_code;
    $Hotels->hotel_short_name=$request->hotel_short_name;
    $Hotels->meeting_description=$request->meet_event_desc;
    $Hotels->seasonal_rates=$request->seasonal_rates;
    $Hotels->image_1=$fd_uploadFile;
    $Hotels->image_2=$fd_uploadFile1;
    $Hotels->image_3=$fd_uploadFile2;
    $Hotels->image_4=$fd_uploadFile3;
    $Hotels->image_5=$fd_uploadFile4;
    $Hotels->save();
    $hl_id=$Hotels->id;
    
    $Hoteladdress = new HodelAddress;
    $Hoteladdress->hl_id=$hl_id;
    $Hoteladdress->address_1=$request->address_1;
    $Hoteladdress->address_2=$request->address_2;
    $Hoteladdress->address_3=$request->address_3;
    $Hoteladdress->country=$request->country;
    $Hoteladdress->state=$request->state;
    $Hoteladdress->city=$request->city;
    $Hoteladdress->postcode=$request->zip_code;
    $Hoteladdress->latitude=$request->latitude;
    $Hoteladdress->longitude=$request->longitude;
    $Hoteladdress->save();

    $HotelMC = new HotelMainContacts;
    $HotelMC->hl_id=$hl_id;
    $HotelMC->main_phone=$request->main_phone;
    $HotelMC->second_phone=$request->second_phone;
    $HotelMC->fax=$request->fax;
    $HotelMC->email=$request->email_hl;
    $HotelMC->url=$request->url;
    $HotelMC->r_d_email=$request->r_d_email;
    $HotelMC->reservation_email=$request->reservation_email;
    $HotelMC->cc_retrieval=$request->cc_retrieval;
    $HotelMC->inventory_notification=$request->inventory_notification;
    $HotelMC->save();

    // $HotelAC = new HotelAdditionalContacts;
    // $HotelAC->hl_id=$hl_id;
    // $HotelAC->c_title=$request->c_title;
    // $HotelAC->f_name=$request->f_name;
    // $HotelAC->l_name=$request->l_name;
    // $HotelAC->job_title=$request->job_title;
    // $HotelAC->email_add=$request->email_add;
    // $HotelAC->c_number=$request->c_number;
    // $HotelAC->m_number=$request->m_number;
    // $HotelAC->cotact_purpose=$request->cotact_purpose;
    // $HotelAC->login_req=$request->login_req;
    // $HotelAC->save();
    ...
    $HotelOI = new HotelOtherInfo;
    $HotelOI->hl_id=$hl_id;
    $HotelOI->currency=$request->currency;
    $HotelOI->t_zone_country=$request->t_zone_country;
    $HotelOI->t_zone_location=$request->t_zone_location;
    $HotelOI->t_zone_offset=$request->t_zone_offset;
    $HotelOI->save();
    ...
    $HotelPA = new HotelPropertyAttributes;
    $HotelPA->hl_id=$hl_id;
    $HotelPA->property_type=$request->property_type;
    $HotelPA->property_type2=$request->property_type2;
    $HotelPA->property_type3=$request->property_type3;
    $HotelPA->alert_email=$request->alert_email;
    $HotelPA->property_email=$request->property_email;
    $HotelPA->guest_email=$request->guest_email;
    $HotelPA->res_del_email=$request->res_del_email;
    $HotelPA->res_del_fax=$request->res_del_fax;
    $HotelPA->price=$request->price;
    $HotelPA->save();

    $HotelAI = new HotelAreaInfo;
    $HotelAI->hl_id=$hl_id;
    $HotelAI->area_attractions=$request->area_attractions;
    $HotelAI->corporate_locations=$request->corporate_locations;
    $HotelAI->h_loc_desc=$request->h_loc_desc;
    $HotelAI->save();

    $HotelMP = new HotelMealPlan;
    $HotelMP->hl_id=$hl_id;
    $HotelMP->off_site_entertainment=$request->off_site_entertainment;
    $HotelMP->off_site_restaurants=$request->off_site_restaurants;
    $HotelMP->on_site_entertainment=$request->on_site_entertainment;
    $HotelMP->special_events=$request->special_events;
    $HotelMP->weddings=$request->weddings;
    $HotelMP->alternate_hotels=$request->alternate_hotels;
    $HotelMP->awards=$request->awards;
    $HotelMP->frequent_guest_information=$request->frequent_guest_information;
    $HotelMP->gds_data=$request->gds_data;
    $HotelMP->handicap_facilities=$request->handicap_facilities;
    $HotelMP->key_selling_points=$request->key_selling_points;
    $HotelMP->miscellaneous=$request->miscellaneous;
    $HotelMP->safety=$request->safety;
    $HotelMP->tour_operators=$request->tour_operators;
    $HotelMP->travel_agent_commission=$request->travel_agent_commission;
    $HotelMP->chef=$request->chef;
    $HotelMP->director_of_sales_and_management=$request->director_of_sales_and_management;
    $HotelMP->fb_director=$request->fb_director;
    $HotelMP->general_manager=$request->general_manager;
    $HotelMP->managing_director=$request->managing_director;
    $HotelMP->reservations_manager=$request->reservations_manager;
    $HotelMP->rooms_division=$request->rooms_division;
    $HotelMP->save();

    $HotelPolicy = new HotelPolices;
    $HotelPolicy->hl_id=$hl_id;
    $HotelPolicy->cancellation_policy=$request->cancellation_policy;
    $HotelPolicy->extended_stay_policy=$request->extended_stay_policy;
    $HotelPolicy->extra_charge_policy=$request->extra_charge_policy;
    $HotelPolicy->family_plan_policy=$request->family_plan_policy;
    $HotelPolicy->general_policy=$request->general_policy;
    $HotelPolicy->group_policy=$request->group_policy;
    $HotelPolicy->guarantee_policy=$request->guarantee_policy;
    $HotelPolicy->pet_policy=$request->pet_policy;
    $HotelPolicy->taxs_policy=$request->taxs_policy;
    $HotelPolicy->save();

    $HotelLPolicy = new HotelLocalPolices;
    $HotelLPolicy->hl_id=$hl_id;
    $HotelLPolicy->check_in_hour=$request->check_in_hour;
    $HotelLPolicy->check_in_minute=$request->check_in_minute;
    $HotelLPolicy->check_out_hour=$request->check_out_hour;
    $HotelLPolicy->check_out_minute=$request->check_out_minute;
    $HotelLPolicy->extra_person_fee=$request->extra_person_fee;
    $HotelLPolicy->extra_child_fee=$request->extra_child_fee;
    $HotelLPolicy->crib_charge=$request->crib_charge;
    $HotelLPolicy->adults_only_hotel_no_kids_allowed=$request->adults_only_hotel_no_kids_allowed;
    $HotelLPolicy->kids_stay_free=$request->kids_stay_free;
    $HotelLPolicy->early_checkout=$request->early_checkout;
    $HotelLPolicy->late_checkout=$request->late_checkout;
    $HotelLPolicy->pet_policy=$request->pet_policy;
    $HotelLPolicy->pets_allowed=$request->pets_allowed;
    $HotelLPolicy->Pets_free=$request->Pets_free;
    $HotelLPolicy->family_policy=$request->family_policy;
    $HotelLPolicy->commission_policy=$request->commission_policy;
    $HotelLPolicy->commission_percentage=$request->commission_percentage;
    $HotelLPolicy->deposit_policy=$request->deposit_policy;
    $HotelLPolicy->cancellation_policy=$request->cancellation_policy;
    $HotelLPolicy->save();

    $HotelProperty = new HotelProperty;
    $HotelProperty->hl_id=$hl_id;
    $HotelProperty->property_check_in_out_desc=$request->property_check_in_out_desc;
    $HotelProperty->property_description=$request->property_description;
    $HotelProperty->property_description_typical=$request->property_description_typical;
    $HotelProperty->property_detail=$request->property_detail;
    $HotelProperty->seasonal_closure=$request->seasonal_closure;
    $HotelProperty->selling_feature_1=$request->selling_feature_1;
    $HotelProperty->selling_feature_2=$request->selling_feature_2;
    $HotelProperty->selling_feature_3=$request->selling_feature_3;
    $HotelProperty->arrival_room_service=$request->arrival_room_service;
    $HotelProperty->business_service=$request->business_service;
    $HotelProperty->facilities=$request->facilities;
    $HotelProperty->health_club_or_fitness=$request->health_club_or_fitness;
    $HotelProperty->local_information=$request->local_information;
    $HotelProperty->meeting_facilities=$request->meeting_facilities;
    $HotelProperty->recreation=$request->recreation;
    $HotelProperty->room_amenities=$request->room_amenities;
    $HotelProperty->services=$request->services;
    $HotelProperty->shopping_local_attraction=$request->shopping_local_attraction;
    $HotelProperty->spa=$request->spa;
    $HotelProperty->directions=$request->directions;
    $HotelProperty->parking=$request->parking;
    $HotelProperty->tourist_information=$request->tourist_information;
    $HotelProperty->transfer_information=$request->transfer_information;
    $HotelProperty->transportation=$request->transportation;
    $HotelProperty->weather_information=$request->weather_information;
    $HotelProperty->save();

    $HotelGDS = new HotelGdsCodes;
    $HotelGDS->hl_id=$hl_id;
    $HotelGDS->amadues=$request->amadues;
    $HotelGDS->galileo=$request->galileo;
    $HotelGDS->sabre=$request->sabre;
    $HotelGDS->worldspan=$request->worldspan;
    $HotelGDS->lanyon=$request->lanyon;
    $HotelGDS->pegasus=$request->pegasus;
    $HotelGDS->save();

for($i=0; $i < count($request->gdsother); $i++){
    $HotelGdsCodesOther= new HotelGdsCodesOther;
    $HotelGdsCodesOther->hl_id=$hl_id;
    $HotelGdsCodesOther->title=$request->gdsother[$i];

}

    // $HotelTaxConfiguration = new HotelTaxConfiguration;
    // $HotelTaxConfiguration->hl_id=$hl_id;
    // $HotelTaxConfiguration->tax_level=$request->tax_level;
    // $HotelTaxConfiguration->tax_frequency=$request->tax_frequency;
    // $HotelTaxConfiguration->tax_type=$request->tax_type;
    // $HotelTaxConfiguration->tax_code=$request->tax_code;
    // $HotelTaxConfiguration->tax_name=$request->tax_name;
    // $HotelTaxConfiguration->tax_desc=$request->tax_desc;
    // $HotelTaxConfiguration->start_date=date('Y-m-d',strtotime($request->start_date));
    // $HotelTaxConfiguration->end_date=date('Y-m-d',strtotime($request->end_date));
    // $HotelTaxConfiguration->no_end_date=$request->no_end_date;
    // $HotelTaxConfiguration->tax=$request->tax;
    // $HotelTaxConfiguration->tax_type_price=$request->tax_type_price;
    // $HotelTaxConfiguration->charge_per=$request->charge_per;
    // $HotelTaxConfiguration->cal_package_price=$request->cal_package_price;
    // $HotelTaxConfiguration->cal_ext_rate=$request->cal_ext_rate;
    // $HotelTaxConfiguration->cal_room_price=$request->cal_room_price;
    // $HotelTaxConfiguration->elg_dis_exclusion=$request->elg_dis_exclusion;
    // $HotelTaxConfiguration->tax_inclusive=$request->tax_inclusive;
    // $HotelTaxConfiguration->apply_free_night=$request->apply_free_night;
    // $HotelTaxConfiguration->tax_by_price_range=$request->tax_by_price_range;
    // $HotelTaxConfiguration->tax_by_los=$request->tax_by_los;
    ...
    $floor_plan = $request->file('floor_plan');
     ...
     if($floor_plan){
        ...
        $floor_plan->move($destinationPath, $imagename);

        copy($destinationPath.'/'.$imagename, $crmdestinationPath.'/meeting/'.$imagename);
        copy($destinationPath.'/'.$imagename, $webdestinationPath.'/meeting/'.$imagename);
     }
    $ddr_packages = $request->file('ddr_packages');

     if($ddr_packages){
        $ddr_packages = $request->file('ddr_packages');
        $imagename = time().'.'.$ddr_packages->getClientOriginalExtension();   
        $destinationPath = public_path().'\uploads\pdf';
        $ddr_packages->move($destinationPath, $imagename);
        copy($destinationPath.'/'.$imagename, $crmdestinationPath.'/pdf/'.$imagename);
        copy($destinationPath.'/'.$imagename, $webdestinationPath.'/pdf/'.$imagename);
     }
    if($request->benefits!=''){
    ...
    return redirect('/hotelier/hotel-portal')->with('message','Hotel Portal Details Added Successfully');  
    }
...
//dd($CategoryPropertyType);
       ...
       $HotelAdditionalContact= HotelAdditionalContacts::where('hl_id',$hl_id)->get();
       ...
       $HotelTaxConfigur= HotelTaxConfiguration::where('hl_id',$hl_id)->get();
...
//dd($Currencylist);
       ...
       //$HotelAdditionalContacts=hotelAdditionalContacts::where('hl_id',$hl_id)->get();

        $portalUsers = HotelAdditionalContacts::
        select('*', 'hotel_additional_contacts.created_at as login_created','hotel_additional_contacts.id as ptid','users.id as uid')        
        ->leftJoin('users', 'users.hotelier_cont_id', '=', 'hotel_additional_contacts.id')
        ->where('hotel_additional_contacts.hl_id',$id)
        ->get();
 //DB::enableQueryLog();
        $CrmUser = User::select('*', 'users.created_at as login_created','users.id as uid')->leftJoin('role_user', 'role_user.user_id', '=', 'users.id')
        ->where('role_user.role_id',4)->get();
...
//dd(DB::getQueryLog());
        ...
        $CategoryRoomsFeatures =CategoryRoomsFeatures::get();
        $CategoryRoomsView =CategoryRoomsView::get();
        

      return view('panels.hotelier.add_hotel_configuration',compact('CategoryRoomsView','CategoryRoomsFeatures','Languages','HotelGroup','CrmUser','portalUsers','task_type','regions','users','events','user_id','Hotelsynxiscrs','HotelLocalPolices','HotelPolices','UserVenueBenefits','UserVenueAmenities','UserVenueBusiness','UserVenueFeatures','UserVenueFooddrink','UserVenueLicensing','HotelGdsCodesOther','states','cities','RoomClass','RoomBedType','RoomsCategory','Currencylist','country','Zone','Airports','CategoryTaxType','CategoryMealPlan','CategoryPropertyType','CategoryLocationType','Rooms','HodelAddress','HotelMainContacts','HotelAdditionalContact','HotelAirportInfo','HotelOtherInfo','HotelPropertyAttributes','HotelAreaInfo','HotelDiningEntertainmentRestaurant','HotelDiningEntertainmentResOpenTime','HotelDiningEntertainmentBreakOpenTime','HotelDiningEntertainmentLunchOpenTime','HotelDiningEntertainmentDinnerOpenTime','HotelDiningEntertainmentBar','HotelDiningEntertainmentGuestOpenTime','HotelDiningEntertainmentPublicOpenTime','HotelMealPlan','HotelPolices','HotelProperty','HotelGdsCodes','HotelRoomsAttributes','HotelRoomsBedType','HotelRoomsOccupancy','HotelRoomsPriceAdjustment','HotelTaxConfigur','VenueCapacity','view_users','id'),['Hotels'=>$Hotels,'Benefits'=>$Benefits,'Amenities'=>$Amenities,'Business'=>$Business,'Category'=>$Category,'VenueContact'=>$VenueContact,'VenueType'=>$VenueType,'VenueFeatures'=>$VenueFeatures,'VenueFoodDrink'=>$VenueFoodDrink,'VenueLicensing'=>$VenueLicensing,'VenueRestrictions'=>$VenueRestrictions,'Edit_VenueCapacity'=>$VenueCapacity,'Edit_VenueFeatures'=>$VenueFeatures,'Edit_VenueFoodDrink'=>$VenueFoodDrink,'Edit_VenueLicensing'=>$VenueLicensing,'Edit_VenueRestrictions'=>$VenueRestrictions,'users' => $users,'editusers' => $users,'editcountry' => $country,'editstates' => $states,'editcities' => $cities,'hotelId'=>$hotelId]);
...
public function getcharts(Request $request)
{
  ...
  $business_id = $request->hotel_mid;
  
  $Hotels= Hotels::where('id',$business_id)->first();

  $userHotelId = $Hotels->id;

  $arrayId1 = DB::table('hotel_leads_events')->pluck('hl_id','hle_id');
  foreach ($arrayId1 as $key => $value) {
    $hlData1 = explode(',', $value);
    if(in_array($business_id, $hlData1)){
      $leadsId1[] =  $key;
    }
  }
  ...
  foreach ($uniqueRegion as $key => $value) {
    ...
    $reg_chart_data[$value] = $reg_chart_data[$value]->whereIn('hle_id', $leadsId1)->get();
  }
...
DB::enableQueryLog();
  ...
  $chart_data = $chart_data->whereIn('hle_id', $leadsId1)->get(); 
  $chart_all_data = $chart_all_data->whereIn('hle_id', $leadsId1)->get();   
//dd(DB::getQueryLog());
// Testing Start
...
public function exportcharts1(Request $request, $type)
{
  ...
  $userHotelId = $request->hotelId;
  
  $arrayId1 = DB::table('hotel_leads_events')->pluck('hl_id','hle_id');
  foreach ($arrayId1 as $key => $value) {
    $hlData1 = explode(',', $value);
    if(in_array($userHotelId, $hlData1)){
      $leadsId1[] =  $key;
    }
  }
  ...
  foreach ($clientType as $keyType1 => $clientType1) {
   ...
   foreach($accountName as $key => $value){
    ...
    foreach($clientNames as $key => $value){
      $client_name = implode(",",$value);
      $chart_all_data[$key]->clientName = $client_name;
      //dd($chart_all_data[$key]->clientName);
    }
...
//     }
  public function emailExist(Request $request){
        ...
        $mail_cnt=User::where(['email'=>$request->mail_val,'activated'=>1])->count();
        
        return response()->json($mail_cnt);
  }
...
if($business >= 3){
    ...
    // Add Hotel Portal Contact User

    public function addedHotelPortalContact(Request $request)
...
$HotelAC = new HotelAdditionalContacts;
    $HotelAC->hl_id=$request->hotel_id;
    $HotelAC->c_title=$request->c_title;
    $HotelAC->f_name=$request->f_name;
    $HotelAC->l_name=$request->l_name;
    $HotelAC->job_title=$request->job_title;
    $HotelAC->email_add=$request->email;
    $HotelAC->c_number=$request->c_number;
    $HotelAC->m_number=$request->m_number;
    $HotelAC->cotact_purpose=$request->cotact_purpose;
    $HotelAC->login_req=$request->login_req;
    $HotelAC->save();
$hotelierContactid=$HotelAC->id;

        
            
if($request->login_req='Yes'){
           ...
           // $photo = $request->file('photo');
                ...
                $user->hotelier_cont_id=$hotelierContactid;                   
                ...
                $data1 = ['email' => $request->email,'firstname' => $request->f_name,'lastname' => $request->l_name,'password' =>$password];
       ...
       //  });
                ...
                $role = Role::whereName('hotelier')->first();                
     ...
     public function updatedHotelPortalContact(Request $request)
...
HotelAdditionalContacts::where('id',$request->cont_id)->update(['hl_id'=>$request->hotel_id,'c_title'=>$request->c_title,'f_name'=>$request->f_name,'l_name'=>$request->l_name,'job_title'=>$request->job_title,'email_add'=>$request->email,'c_number'=>$request->c_number,'m_number'=>$request->m_number,'cotact_purpose'=>$request->cotact_purpose,'login_req'=>$request->login_req]);
$status['id']=$request->loop_id;
$status['status']='success';
...
DB::enableQueryLog();
     DB::table('users')
         ...
         $status['status']='success';
          $status['type']=$request->atype;
   //dd(DB::getQueryLog());      
            return '{"details": ' . json_encode($status) . '}';
}

public function CrmUserAssignPortal(Request $request)
{
$User=User::where('id', $request->id)->first();
if($User->business_id !=''){
$variable=$User->business_id.','.$request->hotel_id;
}else{
    $variable=$request->hotel_id;
}
...
public function UserWelcomeLetterSend(Request $request)
    ...
    $data1 = ['email' => $email,'firstname' => $first_name,'lastname' => $last_name];

                 
        Mail::send('emails.parner_hotel_welcome_letter', $data1, function ($message) use ($data1)
        {
          //dd($data['email_add']);
        $message->from('info@ap-lbc.com', 'ap-lbc.com');
        $message->to($data1['email']);
        //$message->cc('linda@ap-lbc.com');
       // $message->bcc('admin@ap-lbc.com');
        $message->subject("Welcome to APLBC Family");
  ...
  public function UserPasswordSend(Request $request)

    {

        $user = User::where('hotelier_cont_id',$request->id)->first();
        ...
        User::where('id',$user->id)->update(['password'=>$upassword]);


        $data1 = ['email' => $user->email,'firstname' => $user->first_name,'lastname' => $user->last_name,'password' =>$password];


            Mail::send('emails.create_user_portal', $data1, function ($message) use ($data1)
            {
                //dd($data['email_add']);
                $message->from('info@ap-lbc.com', 'ap-lbc.com');
                $message->to($data1['email']);
                //$message->cc('linda@ap-lbc.com');
                // $message->bcc('admin@ap-lbc.com');
                $message->subject("APLBC Hotel Portal Username Password");
...
{    
    ...
    $hlid = $Rooms->id;
if($request->add_key_features!=''){
    $key_features=explode(',', $request->add_key_features);
    foreach ($key_features as $value) {
        $HotelRoomsAttributes = new HotelRoomsKeyFeatures;               
        $HotelRoomsAttributes->hl_room_id=$hlid;                
        $HotelRoomsAttributes->kf_id=$value;  
        $HotelRoomsAttributes->save();  
    }
}

if($request->add_room_view!=''){

    $room_view=explode(',', $request->add_room_view);
    foreach ($room_view as $val) {
        $HotelRoomsView = new HotelRoomsView;               
        $HotelRoomsView->hl_room_id=$hlid;                
        $HotelRoomsView->rv_id=$val;  
        $HotelRoomsView->save();  
    }
}

    $crm_path=env('HOTEL_PATH');
    $crmdestinationPath = $crm_path.'/uploads/rooms/';
    $web_path=env('WEB_PATH');
    $webdestinationPath = $web_path.'/uploads/rooms/';
    ...
    if($uploadFile){                
        ...
        $uploadFile->move($destinationPath, $fd_uploadFile);
        copy($destinationPath.'/'.$fd_uploadFile, $crmdestinationPath.'/'.$fd_uploadFile);
        //copy($destinationPath.'/'.$fd_uploadFile, $webdestinationPath.'/'.$fd_uploadFile);
        DB::table('rooms')
        ->where('id', $hlid)
    ...
    if($uploadFile1){                
        ...
        $destinationPath = public_path().'/uploads/rooms';
        $uploadFile1->move($destinationPath, $fd_uploadFile1);
        copy($destinationPath.'/'.$fd_uploadFile1, $crmdestinationPath.'/'.$fd_uploadFile1);
        //copy($destinationPath.'/'.$fd_uploadFile1, $webdestinationPath.'/'.$fd_uploadFile1);
        DB::table('rooms')
        ->where('id', $hlid)
    ...
    if($uploadFile2){                
        ...
        $destinationPath = public_path().'/uploads/rooms';
        $uploadFile2->move($destinationPath, $fd_uploadFile2);
        copy($destinationPath.'/'.$fd_uploadFile2, $crmdestinationPath.'/'.$fd_uploadFile2);
        //copy($destinationPath.'/'.$fd_uploadFile2, $webdestinationPath.'/'.$fd_uploadFile2);
        DB::table('rooms')
        ->where('id', $hlid)
    ...
    if($uploadFile3){                
        ...
        $destinationPath = public_path().'/uploads/rooms';
        $uploadFile3->move($destinationPath, $fd_uploadFile3);
        copy($destinationPath.'/'.$fd_uploadFile3, $crmdestinationPath.'/'.$fd_uploadFile3);
        //copy($destinationPath.'/'.$fd_uploadFile3, $webdestinationPath.'/'.$fd_uploadFile3);
        DB::table('rooms')
        ->where('id', $hlid)
...
$hlid=$request->roomId;
        if($request->add_key_features!=''){
            $key_features=explode(',', $request->add_key_features);
            $hotelAv=HotelRoomsKeyFeatures::where('hl_room_id',$hlid)->delete();    
        ...
        if($request->add_room_view!=''){
            ...
            $hotelAv=HotelRoomsView::where('hl_room_id',$hlid)->delete();
    ...
    if($uploadFile){ 
        ...
        $dlimage = DB::table('rooms')->where('id',  $request->roomId)->first();
        $dlfile= $dlimage->image_1;
        $dlfilename = public_path().'/uploads/rooms/'.$dlfile;
        $crmdlfilename = $crmdestinationPath.$dlfile;
        $webdlfilename = $webdestinationPath.$dlfile;
        File::delete($dlfilename);
        File::delete($crmdlfilename);
        //File::delete($webdlfilename);
...
{    
    ...
    $HotelTaxConfiguration = new HotelTaxConfiguration; 
    $HotelTaxConfiguration->hl_id=$id;
    $HotelTaxConfiguration->tax_level=$request->tax_level;                    
    $HotelTaxConfiguration->tax_frequency=$request->tax_frequency;
    $HotelTaxConfiguration->tax_type=$request->tax_type;                  
    $HotelTaxConfiguration->tax_code=$request->tax_code;
    ...
    $HotelTaxConfiguration->tax_by_los=$request->tax_by_los;
    $HotelTaxConfiguration->save();             
    $hlid=$HotelTaxConfiguration->id;

    $EditTax = DB::table('hotel_tax_configuration')   
    ->where('id', '=',$hlid)
    ->get();
    $status['EditTax']=$EditTax;
    $status['status']='success';        
...
{   
    ...
    $HotelTaxConfiguration = HotelTaxConfiguration::find($request->taxId);; 
    $HotelTaxConfiguration->hl_id=$id;
    $HotelTaxConfiguration->tax_level=$request->tax_level;                    
    $HotelTaxConfiguration->tax_frequency=$request->tax_frequency;
    $HotelTaxConfiguration->tax_type=$request->tax_type;                  
...
{   
    ...
    $HotelRoomsAttributes->save();
    $data='success';
    return '{"details": ' . json_encode($data) . '}'; 
}
...
{   
    $id=$request->id;
    $Hotels = Hotels::where('id',$id)->update(['price'=> $request->starting_price]);    
    $data='success';
    return '{"details": ' . json_encode($data) . '}'; 
}
