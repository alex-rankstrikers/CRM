            ...
            <div class="row">
					...
					<!-- <div class="col-md-12 col-xs-12">
					 ...
					 <div class="col-md-5 col-xs-12">
								<div class="x_panel">
								<form name="actionForm" action="{{url('admin/client-shout-outs/destroy')}}" method="post" onsubmit="return deleteConfirm();"/> 
									<h2>All Client Shout Outs <span class="pull-right"><a href="#" class="btn btn-success btn-xs add_blog"><i class="fa fa-plus"></i> Add </a>  
                      ...
                      <tbody>
					@foreach ($Clientshoutouts as $news)
					...
					<div class="x_panel" id="add_div" style="">
						<h2>Add Client Shout Outs </h2>
						<div class="x_title">
						</div>
						@if(Session::get('message')) <div class="alert alert-success" role="alert">{{Session::get('message')}} </div>@endif 
						<div class="alert alert-success hidden"></div>
								  <!-- Add Form Start-->
						 <form method="POST" action="{{url('admin/client-shout-outs/added')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="form-horizontal">
                            ...
                            <div id="reportArea">
                                <div class="form-group">
                                 ...
                                 <div class="form-group">
                                    ...
                                    <div class="col-sm-6">
                                        <input class="form-control" type="text" name="n_location" id="n_location" value="{{ old('n_location') }}"/>
										<span class="error">{{ ($errors->has('n_location')) ? $errors->first('n_location') : ''}}</span>
                                    </div>
                                </div>
					...
					<div class="x_panel" id="edit_div" style=" display:none">
						<h2>Edit Client Shout Outs </h2>
						<div class="x_title">
						</div>					
								  <!-- Edit Form Start-->
						 <form method="POST" action="{{url('admin/client-shout-outs/updated')}}"  enctype="multipart/form-data" class="form-horizontal">
                       <input type="hidden" value="" name="id" id="edit_id" />
                            <div id="reportArea">
                                ...
                                <div class="form-group">
                                    <label class="col-sm-2 control-label"> Location</label>
                                    <div class="col-sm-6">
                                        <input class="form-control" type="text" name="n_location" id="edit_n_location" />
										<span class="error">{{ ($errors->has('n_location')) ? $errors->first('n_location') : ''}}</span>
                                    </div>
                                </div>
...
// Add Blog
	...
	function add_news(){  
	var host="{{ url('admin/client-shout-outs/get_users') }}";
	var id= $(this).attr('id');  
	$('#edit_div').hide();
	$('#add_div').fadeIn("slow");	
 ...
 $(document).on("click", ".edit_blog", edit_news);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	...
	var host="{{ url('admin/client-shout-outs/getnews/') }}";
	$('#add_div').hide();
...
var image_path=res.view_details.n_image;
	...
	$('#edit_n_location').val(res.view_details.n_location);
 ...
 $(document).on("click", ".delete_blog", deletenews);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/client-shout-outs/deleted/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
