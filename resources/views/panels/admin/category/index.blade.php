            ...
            <div class="row">
					...
					@foreach ($category as $category)
                        <tr class="rm{{ $category->c_id }}">
						 ...
						 <a href="#" class="enable btn btn-warning btn-xs" id="{{ $category->c_id }}">Home Enable </a>
						 @else
							 <a href="#" class="disable btn btn-success btn-xs" id="{{ $category->c_id }}" > Home Disable </a>
						 @endif
                            ...
                            <div id="reportArea">
								...
								<div class="form-group">
                              	...
                              	<div class="form-group">
                                    ...
                                    <div class="col-sm-6">
                                        <textarea class="form-control" name="c_description" id="c_description">{{ old('c_description') }}</textarea>
                                    </div>
                                </div>							
								<input type="hidden" name="_token" value="{{csrf_token()}}">
                       ...
                       <input type="hidden" value="" name="id" id="edit_id" />
                            <div id="reportArea">
                               ...
                               <div class="form-group">
                                    <label class="col-sm-2 control-label">Description</label>
                                    <div class="col-sm-6">
                                        <textarea class="form-control" name="c_description" id="edit_c_description">{{ old('c_description') }}</textarea>
                                    </div>
                                </div>
...
var image_path=res.view_details.c_image;
	...
	$('#edit_c_description').val(res.view_details.c_description);
...
$(document).on("click", ".enable", enable);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/category/enable/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
...
//Change Status Disable
 ...
 $(document).on("click", ".disable", disable);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/category/disable/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
