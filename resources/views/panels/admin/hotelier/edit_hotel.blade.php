...
@section('content')
 ...
 <form id="commentForm" action="{{url('admin/hotels/updated')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="form-horizontal" method="POST">
...
<table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
                      <thead>
                        <tr><th>
						<input type="checkbox" name="check_all" id="check_all" value=""/></th>		  <th>Image</th> 		 
                          <th>Title</th>
                          <th>Action</th>                         
                        </tr>
                      </thead>
					 ...
					 @foreach ($hotels as $hotel)
                        <tr class="rm{{ $hotel->id }}">
                          <td>
						  <input type="checkbox" name="selected_id[]" class="checkbox" value="{{ $hotel->id }}"/>				 	  
						  </td>
						   <td><?php if($hotel->image_1 != ''){
							 		?>
							<img class="img-responsive" src="{{ url('') }}/uploads/venue/thumbnail/{{ $hotel->image_1 }}" alt="" style="width: 60px;">

							<?php }else{
								if($hotel->image_2 !=''){									?>
								
								<img class="img-responsive 2" src="{{ url('') }}/uploads/venue/thumbnail/{{ $hotel->image_2 }}" alt="" style="width: 60px;">
								<?php 
									}else{
									if($hotel->image_3 !=''){ 
									?>
									<img class="img-responsive 3" src="{{ url('') }}/uploads/venue/thumbnail/{{ $hotel->image_3 }}" alt="" style="width: 60px;">					
								<?php 
									}
						 ...
						 <a href="#" class="enable btn btn-warning btn-xs" id="{{ $hotel->id }}"> Enable </a>
						 @else
							 <a href="#" class="disable btn btn-warning btn-xs" id="{{ $hotel->id }}" > Disable </a>
						 @endif
						 ...
						 <a href="javascript:void(0)" data-toggle="modal" data-target="#myModal" class="btn btn-info btn-xs border-radius-none custom-btn-inner offerId" id="{{ $hotel->id }}" > Update Offer</a>
														...
														<a href="javascript:void(0);" class="edit_blog btn btn-info btn-xs border-radius-none custom-btn-inner" id="{{ $hotel->id }}" > Edit Hotel</a>
														
														<a href="javascript:void(0);" class="btn-danger btn btn-xs delete_blog border-radius-none custom-btn-inner" id="{{ $hotel->id }}"> Delete Hotel</a>	
														
														<a href="{{ url('admin') }}/add-room/{{ $hotel->id }}" class="btn btn-success btn-xs border-radius-none custom-btn-inner" > Add Room</a>
														
														
														<a href="{{ url('admin') }}/view-rooms/{{ $hotel->id }}" class="btn btn-info btn-xs border-radius-none custom-btn-inner" > View Rooms</a>
...
<script type="text/javascript">
	...
	if(offer != '' && daterange != ''){
		...
		$('.erroroffer').html('')
		 var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/update-offer/') }}";
	$.ajax({
		type: 'POST',
...
//Change Status Enable
 ...
 $(document).on("click", ".enable", enable);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/hotels/enable/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
...
//Change Status Disable
 ...
 $(document).on("click", ".disable", disable);
  ...
  var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
   var id= $(this).attr('id'); 
	var host="{{ url('admin/hotels/disable/') }}";
	$.ajax({
		type: 'POST',
		data:{'id': id,'_token':CSRF_TOKEN},
		url: host,
