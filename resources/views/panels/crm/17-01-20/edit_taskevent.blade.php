          ...
          <div class="card-body">
			...
			<div class="col-sm-12 col-md-12 col-lg-12">
				 ...
				 @endforeach
				  ...
				  <span><input name="task_for[]" id="task_for_<?php echo $i;?>" type="checkbox" value="{{ $task_for->hl_ms_id }}" class="task_for" onchange="load_task()">{{ $task_for->hl_service_name }}</input></span>		
				  <?php $i++; ?>
				  @endforeach
				  </div>
				  <input type="hidden" name="taskfor" id="taskfor" value="{{ $tasksE->hle_task_for }}">

				  <div class="form-group col-sm-10 col-md-10 col-lg-10 aligncheck" id="listA">
				...
				<!--  <a class="btn btn-lg btn-primary" href="{{ url('crm/signin') }}" role="button" id="connect-button">Connect to Outlook</a> -->
					...
					<div class="form-group col-sm-6 col-md-6 col-lg-6" id="select_all_hotels" style="float: left;margin-top: 10px;" >
					...
					<div class="form-group col-sm-6 col-md-6 col-lg-6" id="select_contact" style="float: left; margin-top: 10px;">
				...
				<div class="form-group col-sm-12 col-md-12 col-lg-12">
				  ...
				  <div class="form-group col-sm-3 col-md-3 col-lg-3">
				  	<input type="hidden" name="starthr_hid" id="starthr_hid" value="{{ date('H',strtotime($tasksE->hle_start_date)) }}">
				  	<input type="hidden" name="startmin_hid" id="startmin_hid" value="{{ date('i',strtotime($tasksE->hle_start_date)) }}">
				  	<input type="hidden" name="endhr_hid" id="endhr_hid" value="{{ date('H',strtotime($tasksE->hle_end_date)) }}">
				  	<input type="hidden" name="endmin_hid" id="endmin_hid" value="{{ date('i',strtotime($tasksE->hle_end_date)) }}">
				 ...
				 @endfor
				  ...
				  <div style="width:75%;    margin-top: 35px;" >
				   ...
				   <select  style="    margin-top: 26px;" aria-label="Repeat frequency" name="hle_recurr_duration" id="hle_recurr_duration" class="form-control required">
			...
			<div class="container">	  
			  <div class="panel-group">
				<div class="panel panel-default">
					...
					<div id="collapse1" class="panel-collapse collapse">
						<div class="panel-body">
							...
							<div class="col-sm-12 col-md-12 col-lg-12">
							  ...
							  <div id="outcome_section" class="row" style="margin-top: 30px;">
								  <div class="form-group col-sm-11 col-md-11 col-lg-11 appto">
								   ...
								   <a href="" style = "" id="clone" class="btn btn-info btn-sm pull-right">Add</a>
									...
									<option value="">Select</option>
									@foreach ($nextstep as $nextstep2)
									<option value="{{ $nextstep2->hl_ns_id }}" >{{ $nextstep2->hl_ns_title }}</option>
									@endforeach
									</select>
							  		</div>
								  <div class="form-group col-sm-6 col-md-6 col-lg-6">
							  ...
							  <div id="outcome_section{{$i}}" class="row" style="margin-top: 30px;">					
								  <div class="form-group col-sm-11 col-md-11 col-lg-11 appto">
									  <label style="vertical-align:text-top;"> Applicable To:</label>
										<select class="form-control js-example-basic-multiple w-100 applicable_to" multiple="multiple"  name="applicable_to_{{$i}}[]" id="applicable_to_{{$i}}" style="width:90%;"> 
											...
											<optgroup label="{{$sleads[$ee]['region']}}">
												@if(count($sleads[$ee]['dataval']) > 0)
													@foreach($sleads[$ee]['dataval'] as $datav)
														<option value="{{$datav->hl_id}}" 
															@foreach ($appli_hotel as $key => $value) 
															<?php //dd($value.$datav->hl_id);?>
															@if($datav->hl_id == $value) selected="selected" onchange="loadhotels('')" @endif @endforeach>{{$datav->hl_name}}</option>
													@endforeach
												@endif
										@endfor
										</optgroup>
								   ...
								   <div id="removelink" class="form-group col-sm-1 col-md-1 col-lg-1">
								 	<div align="right" class="pull-right"><span id=rem{{$i}} onclick="removediv('{{$i}}')" class="mdi mdi-window-close"></span>
								 	</div>
								    </div>
									...
									<option value="0">Select</option>
									@foreach ($nextstep as $nextstep1)
									<option value="{{ $nextstep1->hl_ns_id }}" {{ ( $outcomeV['std_nextsteps']==$nextstep1->hl_ns_id) ? 'selected' : '' }}>{{ $nextstep1->hl_ns_title }}</option>
									@endforeach
									</select>
							  		</div>
								  <div class="form-group col-sm-6 col-md-6 col-lg-6">
								  <label>Outcome:</label>
								 <!--  <input type="text"  name="hle_outcome[]" id="hle_outcome_0" class="tinymce" value="" /> -->
								 <textarea name="hle_outcome[]" id="hle_outcome_{{$i}}" class="tinymce">{{$outcomeV['hl_outcome']}}</textarea>
								  
								  </div>
								  
								  <div class="form-group col-sm-6 col-md-6 col-lg-6">
								  <label>Next Step:</label>
								 <textarea name="hle_next_step[]" id="hle_nextstep_{{$i}}" class="tinymce">{{$outcomeV['hl_nextstep']}}</textarea>
					...
					<a id="deletebut"  href="{{url('crm/deletevent')}}/{{ $tasksE->hle_id }}" class="btn btn-outline-inverse-info btn-lg btn-crm" >Delete</a>&nbsp;&nbsp;
...
//6th 
		var stHr=$("#starthr_hid").val();
	$("#hle_start_hour").val(stHr);
		var stMin=$("#startmin_hid").val();
	$("#hle_start_min").val(stMin);
		var endHr=$("#endhr_hid").val();
	$("#hle_end_hour").val(endHr);
		var endMin=$("#endmin_hid").val();
	$("#hle_end_min").val(endMin);
//7th
