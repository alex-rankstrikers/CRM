...
<meta name="csrf-token" content="{{ csrf_token() }}" />
         <div class="row mt-30 "></div>	
          ...
          <div class="card-body">
			...
			<h4 class="card-title">View &nbsp;&nbsp;<a href="{{ url('crm/events') }}"><i class="fa fa-refresh" aria-hidden="true"></i></a></h4>
			<div id='leftbar'>
			<?php $i=1;?>
			@foreach ($task_types1 as $task_types2)
			<div class="form-check form-check-{{ $task_types2->task_color }}">
				<label class="form-check-label">
					<input type="checkbox" class="form-check-input viewbox" name="task_types" value="{{ $task_types2->hl_mt_id }}" >{{ $task_types2->task_name }}
				</label>
			</div>
			<?php $i++;?>
			@endforeach
			 ...
			 <div class="form-group flex-items rightalign">
			 <div class="form-group">
			  <label> Select User:</label>
			  <select id="hle_users" name="hle_users[]" multiple class="form-control hle_users"  style="width: auto;">
			  @foreach ($users as $users)
			  @if( Auth::user()->first_name  ==  $users->first_name )
			  <option selected value="{{ $users->user_id }}" >{{ $users->first_name }}</option>
			  @else
			  <option  value="{{ $users->user_id }}" >{{ $users->first_name }}</option>
		      @endif
			  @endforeach
			</select>
			  </div>

			  <div style="text-align:right"> <a href="{{url('crm/create')}}" class="btn btn-primary btn-lg" id="addOffer">Add Task</a></div>
			  </div>
			...
			<div class="card-body" >
			    ...
			    <!-- <div id="treeview_container" class="hummingbird-treeview"> -->
				   ...
				   @foreach($today_tasks as $today_tasks)
				   <li><i class="fa fa-tasks" aria-hidden="true"></i>&nbsp;&nbsp;&nbsp;
	{{$today_tasks->task_name}} &nbsp;&nbsp;&nbsp;<b><a href="">{{$today_tasks->hle_title}}</a></b><div>{{(date('d M , H:i', strtotime($today_tasks->hle_start_date)))}}&nbsp;-&nbsp;{{(date('H:i', strtotime($today_tasks->hle_end_date)))}}&nbsp; for <a>{{$today_tasks->assignee}}</a></div></li>
				    @endforeach
				   </ul>
			</div>
			</div>
		...
		<div class="modal-dialog" role="document">
            ...
            <input type="hidden" name="outlook_id" id="outlook_id" value="" />
			...
			<div class="container">	  
			  <div class="panel-group">
				...
				<div class="modal-footer">
					...
					<a id="deletebut"  href="{{url('crm/deletevent?id=')}}" class="btn btn-outline-inverse-info btn-lg btn-crm" >Delete</a>&nbsp;&nbsp;
					<a id="completebut"  class="btn btn-outline-inverse-info btn-lg btn-crm" href="{{url('crm/completevent?id=')}}">Complete Task</a>
					</div>
...
events : [
                ...
                @foreach($tasks as $task)
                {
					...
					id: '{{ $task->hle_id }}',
                    title : '{{ $task->hle_title }}',
                    searchbar_agents:'@foreach ($search as $value){{$value["searchbar_agents"]}},@endforeach',
                    hle_guests:'@foreach ($search as $value){{$value["hle_guests"]}},@endforeach',
                    hle_guest_id:'@foreach ($search as $value){{$value["hle_guest_id"]}},@endforeach',
                    hl_ocns_id:'@foreach ($ocnss as $ocns){{$ocns["hl_ocns_id"]}},@endforeach',
                    applicable_to:'@foreach ($ocnss as $ocns){{$ocns["applicable_All_Hotels"]}}~@endforeach',
                    std_outcome:'@foreach ($ocnss as $ocns){{$ocns["std_outcomes"]}},@endforeach',
                    std_nextstep:'@foreach ($ocnss as $ocns){{$ocns["std_nextsteps"]}},@endforeach',
                    hle_outcome:'@foreach ($ocnss as $ocns){{$ocns["hl_outcome"]}},@endforeach',
                    hle_nextstep:'@foreach ($ocnss as $ocns){{$ocns["hl_nextstep"]}},@endforeach',
                    outlook_id: '{{ $task->outlook_id }}',
                    description : '{{ $task->hle_description }}',
                    allhotel : '{{ $task->hle_applicable_All_Hotels }}',
                    allcontact : '{{ $task->hle_applicable_contacts }}',
                    category : '{{ $task->hle_category }}',
                    task_for : '{{ $task->hle_task_for }}',
                    activity : '{{ $task->hle_activity }}',
                    start_hour : '{{ date("H", strtotime($task->hle_start_date)) }}',
                    start_min : '{{ date("i", strtotime($task->hle_start_date)) }}',
                    end_hour : '{{ date("H", strtotime($task->hle_end_date)) }}',
                    end_min : '{{ date("i", strtotime($task->hle_end_date)) }}',
                    task_type : '{{ $task->hle_task_type }}',
                    hle_recurr_status : '{{ $task->hle_recurr_status }}',
                    show_location : '{{ $task->show_location }}',
                    recurr_cnt : '{{ $task->hle_recurr_cnt }}',
                    recurr_duration : '{{ $task->hle_recurr_duration }}',
                    hle_timezone : '{{ $task->hle_timezone }}',
                    hle_allday_status : '{{ $task->hle_allday_status }}',
                    hle_location : '{{ $task->hle_location }}',
                    hle_lead : '{{ $task->hl_lead }}',
                    start : '{{ $task->hle_start_date }}',
					end: '{{ $task->hle_end_date }}',
					
					@if($task->hle_task_type == 'Hosted Events')
						color: '#ff9f89'
...
//alert(hn);
				...
				var urlval_del = "{{url('crm/deletevent')}}";
